    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `Todo` (`id` INTEGER PRIMARY KEY AUTOINCREMENT, `task` TEXT NOT NULL, `key` TEXT NOT NULL, `todoDate` INTEGER NOT NULL, `status` INTEGER NOT NULL, `tagId` INTEGER NOT NULL, `subtasks` TEXT NOT NULL)');
        await database.execute(
  ...
  Future<List<Todo>> getAllTodos() async {
    return _queryAdapter.queryList('SELECT * FROM Todo',
        mapper: (Map<String, Object?> row) => Todo(
            ...
            subtasks: _subtaskListConverter.decode(row['subtasks'] as String)));
  }
